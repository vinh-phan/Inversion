...
run_2sls_specs <- function(df, ss_out_file, fs_out_file) {
  ...
  for (i in seq_len(nrow(df_filtered_specs))) {
    ...
    mod <- feols(as.formula(spec$formula), data = df_use, weights = df_use$lassoweight)
...
run_var_specs <- function(df, out_file) {
  ...
  for (i in seq_len(nrow(df_filtered_specs))) {
    ...
    mod <- feols(as.formula(formula), data = df_use, weights = df_use$lassoweight)
    ...
    mod <- feols(as.formula("Yt ~ Zt"), data = df_use, weights = df_use$lassoweight)
